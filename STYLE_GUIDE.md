...
> Conventions and formatting rules for contributing to Hermes How To. Follow this guide to keep content consistent, professional, and easy to maintain.
...
|------|-----------|----------|
| **Lesson README** | `README.md` | `01-quickstart/README.md` |
| **Feature file** | Kebab-case `.md` | `agent-setup.md`, `memory-config.md` |
| **Shell script** | Kebab-case `.sh` | `setup-agent.sh`, `validate-config.sh` |
...
| **Image assets** | Kebab-case | `delegation-flow.png`, `hermes-howto-logo.svg` |
...
Individual feature files (e.g., `workflows.md`, `providers.md`):
...
| Level | Use | Example |
|-------|-----|---------|
| `#` H1 | Page title (one per document) | `# Delegation` |
| `##` H2 | Major sections | `## Best Practices` |
| `###` H3 | Subsections | `### Creating a Subagent` |
...
```markdown
> **Note**: Subagents inherit context from the parent agent.

> **Important**: Always verify permissions before delegation.

> **Tip**: Combine memory with delegation for maximum effectiveness.
```
...
```markdown
- **Delegation patterns** - identify when to delegate, what to delegate
- **Context propagation** - understand what gets passed to subagents
- **Permission management** - control what subagents can access
```
...
|---------|-----------|------------|----------|
| **Delegation** | Manual (`/delegate`) | Session only | Complex tasks |
...
| Aspect | Details |
|--------|---------|
| **Purpose** | Delegate tasks to subagents |
| **Scope** | Task-level |
...
hermes delegate --agent research --task "analyze market trends"
```
...
cp 04-delegation/*.md .claude/delegation/
```
...
[Delegation Guide](04-delegation/)
...
```markdown
[Hermes Agent Documentation](https://docs.hermes.ai/overview)
```
...
- [Quick Start](../01-quickstart/) - Getting started
...
### Feature Files (Agents, Skills, Tasks)
...
Use responsive images with alt text:

```markdown
![Description of image](./assets/diagram.png)
```

For logos with dark/light variants:

```html
<picture>
  <source media="(prefers-color-scheme: dark)" srcset="./assets/logo-dark.svg">
  <img alt="Hermes How To" src="./assets/logo.svg">
</picture>
```
...
- Use SVG over PNG for diagrams when possible
...
- **Active voice** — "The agent sends a request" not "A request is sent"
- **Second person** — "You can configure" not "Users can configure"
- **Present tense** — "The feature works" not "The feature worked"
...
Examples:
```
feat(delegation): Add subagent permission patterns
docs(memory): Improve context preservation examples
fix(quickstart): Correct installation commands
...
- [ ] Document has proper structure (H1 → H2 → H3)
- [ ] Code blocks have language tags
- [ ] Links are relative for internal, absolute for external
- [ ] Tables are properly formatted
- [ ] No decorative emojis in body text
- [ ] Mermaid diagrams are simple and valid
